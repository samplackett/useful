...
DECLARE
    ...
    v_Value INT;
BEGIN
    FOR v_Row IN SELECT * FROM temp_pr_values LOOP
        v_CorrelationId := v_Row.CorrelationId;
        v_FRN := v_Row.FRN;
        v_InvoiceNumber := v_Row.InvoiceNumber;
        v_Value := v_Row.Value;

        IF EXISTS (SELECT 1 FROM public."reportData" WHERE "correlationId" = v_CorrelationId AND frn = v_FRN AND "invoiceNumber" = v_InvoiceNumber) THEN
            UPDATE public."reportData"
            SET 
                value = v_Value
            WHERE "correlationId" = v_CorrelationId AND frn = v_FRN AND "invoiceNumber" = v_InvoiceNumber;
        ELSE
    ...
    DROP TABLE IF EXISTS temp_pr_values;
END $$;
